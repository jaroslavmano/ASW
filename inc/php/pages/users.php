<?php
if(!in_array("users_manage",$LoginPermission)){
...
<h1 class="text-center text-4xl text-[<?=$system->SystemSettings["color_2"]?>] font-bold mt-6 mb-6">Správa uživatelů</h1>
...
<div class="text-right mr-[5%]">
	...
	if(in_array("users_createedit",$LoginPermission)){
		?>
<a href="?page=user&status=create" class="rounded-md border border-transparent py-2 px-4 text-sm font-medium shadow-sm bg-[<?=$system->SystemSettings["button_bg"]?>]  text-[<?=$system->SystemSettings["button_color"]?>] align-middle inline-block rounded-md p-3 hover:text-[<?=$system->SystemSettings["button_color:hover"]?>] hover:bg-[<?=$system->SystemSettings["button_bg:hover"]?>] hover:cursor-pointer"><ion-icon name="add" class="inline-block align-middle"></ion-icon>  Vytvořit uživatele</a>
	<?php } ?>
</div>
    ...
    <table class="w-full text-sm text-left text-[<?=$system->SystemSettings["color_1"]?>]">
        ...
        <tbody>
            ...
            <tr class="bg-[<?=$system->SystemSettings["table_body_bg"]?>] text-[<?=$system->SystemSettings["table_body_color"]?>] border-b border-[<?=$system->SystemSettings["table_body_border"]?>]">
                ...
                <td scope="row" class="px-6 py-4">
                    <?=date("d/m/Y",$user["User_BirthDate"])?>
                </td>
                <td class="px-6 py-4">
					<?php
						if(in_array("users_createedit",$LoginPermission)){
							echo '
							<p>
							<a href="?page=user&id='.$user["User_ID"].'&status=edit" class="font-medium text-['.$system->SystemSettings["link_color"].'] hover:text-['.$system->SystemSettings["link_color:hover"].'] hover:underline"><ion-icon name="cog" class="inline-block align-middle text-xl"></ion-icon> '.constant("MANAGE").'</a></p>
							';
						}
						if(in_array("users_remove",$LoginPermission) && $user["User_ID"] != $loginUser->UserData[0]["User_ID"]){
							echo '
							<p>
							<a href="?save=remove&type=user&id='.$user["User_ID"].'" class="font-medium text-['.$system->SystemSettings["link_color"].'] hover:text-['.$system->SystemSettings["link_color:hover"].'] hover:underline"><ion-icon name="trash" class="inline-block align-middle text-xl"></ion-icon> Smazat</a></p>
							';
						}
					?>
                </td>
