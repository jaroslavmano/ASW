...
<div class="container relative mx-auto mt-5 mb-5 ">
  <div class="grid grid-cols-1 m-3 md:grid-cols-2 md:m-2 lg:grid-cols-3 lg:m-1 gap-5">
      ...
      <?php
      $modules = new Module();
      if($modules->VerifyModule("TAG") == 1){
      ?>
          <?=((in_array("tag_manage",$LoginPermission))?'<a href="?page=tags" class="flex justify-center align-middle text-center text-4xl text-['.$system->SystemSettings["color_1"].'] border-2 border-neutral-700 rounded-xl p-6 bg-black">Správa Tagů</a>':'')?>
      <?php
      } else {
          echo "Je vypnutý";
      }
